...
describe("[Challenge] Free Rider", function () {
  ...
  // const PLAYER_INITIAL_ETH_BALANCE = 16n * 10n ** 18n; // 1n * 10n ** 17n;
  ...
  before(async function () {
    ...
    uniswapRouter = await new ethers.ContractFactory(
      routerJson.abi,
      routerJson.bytecode,
      deployer
    ...
    uniswapPair = await new ethers.ContractFactory(
      pairJson.abi,
      pairJson.bytecode,
      deployer
    ).attach(await uniswapFactory.getPair(token.address, weth.address));
    ...
    ).attach(await marketplace.token());
    expect(await nft.owner()).to.eq(ethers.constants.AddressZero); // ownership renounced
    expect(await nft.rolesOf(marketplace.address)).to.eq(
  ...
  it("Execution", async function () {
    ...
    const attacker = await (
      await ethers.getContractFactory(
        "FreeRaiderNFTMarketplaceAttacker",
        player
      )
    ).deploy(
      weth.address,
  ...
  after(async function () {
    ...
    for (let tokenId = 0; tokenId < AMOUNT_OF_NFTS; tokenId++) {
      await nft
        .connect(devs)
        .transferFrom(devsContract.address, devs.address, tokenId);
      expect(await nft.ownerOf(tokenId)).to.be.eq(devs.address);
    }

    // Exchange must have lost NFTs and ETH
    expect(await marketplace.offersCount()).to.be.eq(0);
    expect(await ethers.provider.getBalance(marketplace.address)).to.be.lt(
