...
describe("[Challenge] The rewarder", function () {
  ...
  before(async function () {
    ...
    accountingToken = AccountingTokenFactory.attach(
    ...
    expect(
      await accountingToken.hasAllRoles(
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      await liquidityToken
        .connect(users[i])
        .approve(rewarderPool.address, depositAmount);
      await rewarderPool.connect(users[i]).deposit(depositAmount);
      expect(await accountingToken.balanceOf(users[i].address)).to.be.eq(
        depositAmount
      );
    }
    expect(await accountingToken.totalSupply()).to.be.eq(
      depositAmount * BigInt(users.length)
    );
  ...
  it("Execution", async function () {
    ...
    const attacker = await (
      await ethers.getContractFactory("TheRewarderPoolAttacker", player)
    ).deploy(
    ...
    // 2. Advance time 5 days again so that depositors can get rewards
    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
    await ethers.provider.send("evm_mine");
    expect(await rewarderPool.isNewRewardsRound()).to.be.true;

    // 3. Exploit
    const amount = await liquidityToken.balanceOf(flashLoanPool.address);
    await attacker.connect(player).exploit(amount);
  });
